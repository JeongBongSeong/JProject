...
HRESULT JMesh::CreateConstantBuffer()
{
	HRESULT hr = S_OK;
	hr = JModel::CreateConstantBuffer();
	if (m_pVertexList.size() <= 0) return hr;

	D3D11_BUFFER_DESC bd;
	ZeroMemory(&bd, sizeof(D3D11_BUFFER_DESC));
	bd.ByteWidth = sizeof(JAnimMatrix);
...
HRESULT JMesh::CreateVertexBuffer()
{
	HRESULT hr = S_OK;
	JModel::CreateVertexBuffer();
...
HRESULT JMesh::CreateVertexLayout()
{
	HRESULT hr = S_OK;
...
bool JMesh::PreRender(ID3D11DeviceContext* pContext)
{
