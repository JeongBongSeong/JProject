...
HRESULT JModel::CreateIndexBuffer()
{
    ...
    D3D11_BUFFER_DESC bd;
    ZeroMemory(&bd, sizeof(D3D11_BUFFER_DESC));
    bd.ByteWidth = sizeof(DWORD);
    bd.Usage = D3D11_USAGE_DEFAULT;
    bd.BindFlags = D3D11_BIND_INDEX_BUFFER;
    D3D11_SUBRESOURCE_DATA data;
    ZeroMemory(&data, sizeof(D3D11_SUBRESOURCE_DATA));
    data.pSysMem = &m_pIndexList;
...
bool JModel::Load(std::wstring filename,std::string vsEntry, std::string psEntry)
{
...
bool JModel::PostRender(ID3D11DeviceContext* pContext)
{
    return true;
}
