...
void JFbxObj::SetPixelShader(ID3D11PixelShader* ps)
{
...
TMatrix     JFbxObj::ConvertAMatrix(FbxAMatrix& m)
{
	TMatrix mat;
	float* pMatArray = reinterpret_cast<float*>(&mat);
	double* pSrcArray = reinterpret_cast<double*>(&m);
...
bool	JFbxObj::LoadObject(std::string filename, std::string shaderfilename)
{
	...
	for (int iMesh = 0; iMesh < m_pMeshList.size(); iMesh++)
	{
		...
		{
			JMesh* pLinkMesh = GetFindInedx(pMesh->m_pFbxNodeList[iBone]);
		...
		if (pMesh->m_pSubMesh.size() > 0)
		{
			for (int iSubMesh = 0; iSubMesh < m_pMeshList[iMesh]->m_pSubMesh.size(); iSubMesh++)
			{
				...
				pSubMesh->Create(TBASIS::mtw(shaderfilename), TBASIS::mtw(shaderfilename), "VS","PS",L"");
			}
		}
		else
		{
			pMesh->Create(TBASIS::mtw(shaderfilename), TBASIS::mtw(shaderfilename), "VS", "PS", L"");
		}
	}
	m_pFbxScene->Destroy();
